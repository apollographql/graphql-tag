...
[gqlRequire, gqlDefault].forEach((gql, i) => {
  describe(`gql ${i}`, () => {
    ...
    it('provides an inverse to the string tag', () => {
      const string = '{ field(input: "1") }';
      assert.equal(gqlRequire.stringify(gql`${string}`), string);
    });

    it('provides an inverse to gql string tag on formatted strings', () => {
      const fragment = `fragment SomeFragment on SomeType { someField }`
      const query = `query { someQuery { ...SomeFragment } } `
      const gqlFragment = gql`${fragment}`
      const gqlQuery = gql`${query}${gqlFragment}`;
      assert.equal(gqlRequire.stringify(gqlQuery), `${query}${fragment}`);
    });
